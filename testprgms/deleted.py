...
def stopword(strf):
    ...
    # print("hai")
    # print(text5)
    #print(keywords,keywordsnew)
    # keywords=list(dict.fromkeys(keywords))
    #np.char.lower(keywords)
    #print(keywords)
...
def tfidf(text,sttnew):
    ...
    # print("tfidf")
    for s in sttnew:
        #for k in distros_dict[s]:
            #print(k)
        vocabulary=vocabulary+(distros_dict[s]["best"])
        #for m in distros_dict[s]
    cbest=len(vocabulary)
    # print(cbest)
    for s in sttnew:
        vocabulary=vocabulary+(distros_dict[s]["average"])
    cavg=len(vocabulary)
    # print(cavg)
    for s in sttnew:
        vocabulary=vocabulary+(distros_dict[s]["common"])
    ccom=len(vocabulary)
    # print(ccom)
    # print(vocabulary)
...
def plagiarism(strf):
    ...
    #                 print(doc1,doc2)
    #
def grammarcheck(strf,co):
    tool = language_check.LanguageTool('en-US')
    document = []
    for filename in strf:
        # print(filename)
        filename = "D:/AEG/Uploads/" + filename
    ...
    for i in range(len(strf)):
        ...
        w=sf.split(".")
     #   print(w)
        f = open("D:/AEG/Reports/"+w[0]+".txt", "w")
